...
export function SupportPage() {
  ...
  const filteredTopics = useMemo(() => {
    ...
    return supportTopics
      .map((tema) => ({
        ...tema,
        items: tema.items.filter((i) => t(i.q).toLowerCase().includes(term) || t(i.a).toLowerCase().includes(term)),
      }))
  ...
  return (
    <>
      ...
      <Container className="py-10">
        ...
        <aside aria-labelledby="sat-callout" className="mt-8 rounded-[16px] border border-line bg-neutral p-6 md:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-sm text-ink">{t('support.repairBody')}</p>
            </div>
          ...
          {filteredTopics.length === 0 ? (
            ...
            <div className="grid gap-8 md:grid-cols-2">
              {filteredTopics.map((tema) => (
                <div key={tema.topic}>
                  <h3 className="mb-2 text-sm font-bold uppercase tracking-wider text-ink">{t(tema.topic)}</h3>
                  <Accordion items={tema.items.map((i) => ({ q: t(i.q), a: t(i.a), note: t(i.note) }))} />
                </div>
              ))}
            </div>
          )}
